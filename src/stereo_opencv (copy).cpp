...
static void print_help()
{
    ...
    printf("\nUsage: stereo_match <left_image> <right_image> [--algorithm=bm|sgbm|hh|sgbm3way] [--blocksize=<block_size>]\n"
           "[--max-disparity=<max_disparity>] [--scale=scale_factor>] [-i=<intrinsic_filename>]\n"
...
int main(int argc, char** argv)
{
    ...
    std::cout<<"Intrinsic file: "<<intrinsic_filename<<std::endl;
    int color_mode = alg == STEREO_BM ? 0 : -1;
    Mat img1 = imread(img1_filename, color_mode);
    Mat img2 = imread(img2_filename, color_mode);
    std::cout<<"image name: "<<img1_filename<<" , "<<img2_filename<<std::endl;
    if (img1.empty())
    {
    ...
    if( !intrinsic_filename.empty() )
    {
        ...
        for( int k = 0; k < 2; k++ )
        {
            ...
            if(k==0)
                resize(img1, canvasPart,
                    canvasPart.size(), 0, 0, INTER_AREA);
            else
                resize(img2, canvasPart,
                       canvasPart.size(), 0, 0, INTER_AREA);
        }

	//    cv::resize(canvas,canvas,cv::Size(),0.25,0.25);
    ...
    point_cloud_filename = "cloud_cv.xml";
    if(!point_cloud_filename.empty())
    {
        FileStorage fs(point_cloud_filename, FileStorage::WRITE);
        fs<<"disp"<<disp;
        fs<<"cloud"<<xyz;
    }
    viz::Viz3d myWindow("Cloud");
    myWindow.showWidget("Coordinate Widget", viz::WCoordinateSystem(50));
    cv::viz::WCloud cloud_widget(xyz, cv::viz::Color::white());
